  ...
  describe ApplicationStack do
    ...
    describe "launch" do
      it "should launch its instances when launched" do
        s = ApplicationStack.define("test") do |s|
          s.instance :db,  :instance_type => Awsymandias::EC2::InstanceTypes::C1_XLARGE
          s.instance :app, :instance_type => Awsymandias::EC2::InstanceTypes::M1_LARGE
        end
      ...
      it "should attach volumes when launched" do
        s = ApplicationStack.define("test") do |s|
          s.instance :db, :instance_type => Awsymandias::EC2::InstanceTypes::M1_LARGE
      ...
      it "should create volumes from snapshots and attach them to each instance of the appropriate role" do
        s = ApplicationStack.define("test") do |s|
          s.instance :app1, :instance_type => Awsymandias::EC2::InstanceTypes::M1_LARGE, :role => :app
          s.instance :app2, :instance_type => Awsymandias::EC2::InstanceTypes::M1_LARGE, :role => :app
          s.instance :app3, :instance_type => Awsymandias::EC2::InstanceTypes::M1_LARGE, :role => :app
          s.instance :no_attached_volume, :instance_type => Awsymandias::EC2::InstanceTypes::M1_SMALL, :role => :other_role
        ...
        Instance.should_receive(:launch).
          with(hash_including(:instance_type => Awsymandias::EC2::InstanceTypes::M1_LARGE)).exactly(3).times.
    ...
    describe "launch_begun?" do    
      it "should be false initially" do
        s = ApplicationStack.define("test") { |s|  s.instance :db, :instance_type => Awsymandias::EC2::InstanceTypes::M1_LARGE }
    ...
    describe "port_open?" do
      it "should return true if there is one instance with the port open" do
        s = ApplicationStack.define("test") do |s|
          s.instance "app", :instance_type => Awsymandias::EC2::InstanceTypes::C1_XLARGE
        end
        s.should_receive(:store_app_stack_metadata!).any_number_of_times.and_return(nil)
      ...
      it "should return true if there are multiple instances all with the port open" do
        s = ApplicationStack.define("test") do |s| 
          s.instance "app1", :instance_type => Awsymandias::EC2::InstanceTypes::C1_XLARGE
          s.instance "app2", :instance_type => Awsymandias::EC2::InstanceTypes::C1_XLARGE
        end
        s.should_receive(:store_app_stack_metadata!).any_number_of_times.and_return(nil)
    ...
    describe "terminate!" do
      ...
      it "should terminate all instances, remove them from the collection, and add them to the terminated collection if running" do
        s = ApplicationStack.define("test") { |s|  s.instance "db" }
        s.should_receive(:remove_app_stack_metadata!).once.and_return(nil)
        s.should_receive(:store_app_stack_metadata!).any_number_of_times.and_return(nil)
        ...
        mock_instance.should_receive(:name).any_number_of_times.and_return("db")
        mock_instance.should_receive(:terminate!)
        
        Instance.stub!(:launch).and_return(mock_instance)
        s.launch
        s.instance_variable_get("@instances").size.should == 1
        s.instance_variable_get("@terminating_instances").size.should == 0
        s.terminate!      
        s.instance_variable_get("@instances").size.should == 0
        s.instance_variable_get("@terminating_instances").size.should == 1
      end
            
      it "should set the terminating flag" do
        s = ApplicationStack.define("test") { |s|  s.instance "db" }
        s.should_receive(:remove_app_stack_metadata!).once.and_return(nil)
        s.should_receive(:store_app_stack_metadata!).any_number_of_times.and_return(nil)
        ...
        Instance.stub!(:launch).and_return(mock_instance)
        s.launch
        s.instance_variable_get("@terminating").should be_false
        s.terminate!      
        s.instance_variable_get("@terminating").should be_true
      end
            
      it "should terminate all load_balancers if running" do
        s = ApplicationStack.define("test") { |s| s.load_balancer 'test-lb' }
        s.should_receive(:remove_app_stack_metadata!).once.and_return(nil)
        s.should_receive(:store_app_stack_metadata!).any_number_of_times.and_return(nil)
      
        mock_lb = mock("a-load-balancer", :launched? => true)
        mock_lb.should_receive(:name).any_number_of_times
        mock_lb.should_receive(:terminate!)
        LoadBalancer.stub!(:launch).and_return(mock_lb)
    ...
    describe "terminated?" do
      it "should return true if there are no instances and all terminating instances say they're terminated" do
        s = ApplicationStack.new(:some_name)
        s.instance_variable_set "@instances", {}
        
        terminating_instance = mock( :terminated? => true)
        s.instance_variable_set "@terminating_instances", {:foo => terminating_instance}
        s.terminated?.should be_true
      end
      
      it "should return false if there are any instances in the collection" do
        s = ApplicationStack.new(:some_name)
        s.instance_variable_set "@instances", {:foo => :bar}          
        s.instance_variable_set "@terminating_instances", {}
        s.terminated?.should be_false
      end

      it "should return false if there are any instances in the terminating collection that have not terminated" do
        s = ApplicationStack.new(:some_name)
        s.instance_variable_set "@instances", {}          
        terminating_instance = mock( :terminated? => false)
        s.instance_variable_set "@terminating_instances", {:foo => terminating_instance}
